...
class Engine():
    def __init__(self, cfg, local_rank: int, world_size: int):
        ...
        # self.val_dataloader = self.data_loader_feactory.build(state="val", **data_json) # 
        ...
        # self.logger_val = utils.logger.create_logger(self.cfg.workshop, name="val") if self.local_rank == 0 else None 
        ...
        # self.val_score_1, self.val_score_2, self.val_score_3, self.val_loss = [], [], [], [] 
...
#     uncomment val function if need cross-validation
#     def val(self):
#         assert self.val_dataloader is not None, print("Val dataloader is not defined")
#         discrip_str = f"Epoch-{self.current_epoch}"
#         pbar_val = tqdm(self.val_dataloader, disable=self.local_rank != 0)
#         pbar_val.set_description("Val" + discrip_str)
...
#                 pbar_val_dic['score'] = f'{self.score_meter.avg:.5f}'
...
#                 average_f1 = 'weighted' if self.cfg.dataset.database == 'meld' else 'macro'
#                 score_1, score_2, score_3, score_4, score_5 = self.calculate_score(epoch_preds, epoch_labels, average_f1)
#                 self.val_score_1.append(score_1)
#                 self.val_score_2.append(score_2)
#                 self.val_score_3.append(score_3)
#                 self.val_loss.append(self.loss_meter.avg)
            
#             if self.logger_val is not None:
#                 self.logger_val.info(
#                     f'Validation epoch: {self.current_epoch}, accuracy: {score_1:.5f}, precision: {score_4:.5f}, recall: {score_2:.5f}, F1: {score_3:.5f}, loss: {self.loss_meter.avg:.5f}, confuse_matrix: \n{score_5}'
#                 )
    ...
    def run(self):
        if self.cfg.train.find_init_lr:
            ...
            while self.current_epoch < self.EPOCH:
                self.train_epoch()
                self.scheduler.step()
                # self.val()  # uncomment if need cross-validation
                self.test()
...
def main_worker(local_rank, cfg, world_size, dist_url):
    ...
    if cfg.dataset.database == 'iemocap':
        ...
        cfg.train.vote = True
        folds = [1]
    elif cfg.dataset.database == "combined":
        folds = [1]
    else:
    ...
    if local_rank == 0:
        ...
        outfile = f'result/result_{cfg.model.type}_{cfg.dataset.database}.csv'

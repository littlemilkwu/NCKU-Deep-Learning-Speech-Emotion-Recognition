...
class combined(Base_database):
    def __init__(self, matdir=None, matkey=None, state=None, meta_csv_file=None, fold=5, seed=11):
        assert state in ['train', "val", 'test'], print(f'Wrong state: {state}')
        ...
        label_converter = {'neutral': 0, 'angry': 1, 'happy': 2, 'sad': 3, 'surprise': 4, 'disgust': 5, 'fear': 6, "calm": 7}
        if matdir is not None:
            if self.state == "train" or self.state == "val":
                matdir = os.path.join(matdir, "train")
            else:
                matdir = os.path.join(matdir, "test")
        super().__init__(names, labels, matdir, matkey, state, label_converter)
    ...
    def split_train_test_5fold(self, fold):
        index = list(range(len(self.labels)))
        start, end = int(len(self.labels) * (fold - 1) / 5), int(len(self.labels) * (fold / 5) - 1)
        if self.state == "val":
            # print(index)
            index = list(range(start, end))
            # print("val index: {}".format(index))
        else: # state = train
            exclude_index = list(range(start, end))
            index = [i for i in index if i not in exclude_index]
...
class combined_dataset(Base_dataset):
    def __init__(self, matdir, matkey, state, meta_csv_file, length=0, feature_dim=0,
                pad_value=0, mode="constant", fold=1, strategy="5cv", **kwargs):
        database = combined(matdir, matkey, state, meta_csv_file)
        # if state == "train" or state == "val":       # uncomment if need cross-validation
        #     database.split_train_test_5fold(fold)
        super().__init__(database, mode, length, feature_dim, pad_value)
...
class DataloaderFactory():
    ...
    def build(self, state, **kwargs):
        ...
        elif self.cfg.dataset.database == "combined":
        ...
        return DistributedDataloaderWrapper(dataloader, collate_fn)

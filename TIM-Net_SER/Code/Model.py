...
class TIMNET_Model(Common_Model):
    ...
    def train(self, x, y):
        ...
        with open(resultpath+self.args.data+'_'+str(self.args.split_fold)+'fold_'+str(round(self.acc*10000)/100)+"_"+str(self.args.random_seed)+"_"+now_time+'.json', 'w') as jfile:
            json.dump(dict_history, jfile)
        K.clear_session()
        self.matrix = []
        self.eva_matrix = []
        self.acc = 0
    ...
    # cus
    def test_cus(self, x, y, path):
        x_feats = []
        y_labels = []

        self.create_model()
        weight_path=path+"/5-fold_weights_best_5.hdf5"
        self.model.load_weights(weight_path)    #+source_name+'_single_best.hdf5')
        test_loss, test_accuracy = self.model.evaluate(x, y)

        y_pred_best = self.model.predict(x)
        cm = confusion_matrix(
            np.argmax(y, axis=1), 
            np.argmax(y_pred_best,axis=1))
        pd.DataFrame(cm, columns=self.class_label, index=self.class_label).to_csv('TestConfusionMatrix.csv')
        ...
        feature_source = caps_layer_model.predict(x)
        x_feats.append(feature_source)
        y_labels.append(y)

        print("Test CUS ACC:", test_accuracy)
        return x_feats, y_labels